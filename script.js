        ...
        if (typeof realtime !== 'undefined' && realtime) {
            ...
            realtime.on('order_created', (data) => {
                try {
                    ...
                    // Build left column using textContent to avoid XSS
                    const buyerP = document.createElement('p');
                    buyerP.className = 'mb-1';
                    const buyerStrong = document.createElement('strong');
                    buyerStrong.textContent = '👤 Nama Pembeli:';
                    buyerP.appendChild(buyerStrong);
                    buyerP.appendChild(document.createTextNode(' ' + (order.buyerName || '')));

                    const tableP = document.createElement('p');
                    tableP.className = 'mb-1';
                    const tableStrong = document.createElement('strong');
                    tableStrong.textContent = '🪑 Nomor Meja:';
                    tableP.appendChild(tableStrong);
                    tableP.appendChild(document.createTextNode(' ' + (order.tableNumber || '')));

                    const idP = document.createElement('p');
                    idP.className = 'mb-0';
                    const idStrong = document.createElement('strong');
                    idStrong.textContent = '📅 ID Pesanan:';
                    ...
                    const h6Icon = document.createElement('i');
                    h6Icon.className = 'bi bi-list-check';
                    h6Icon.style.color = '#00a856';
                    h6Icon.style.marginRight = '6px';
                    h6.appendChild(h6Icon);
                    h6.appendChild(document.createTextNode('Detail Item:'));
                    ...
                    // Set static table header (sanitized)
                    safeSetHTML(table, `<thead style="background-color: #f8f9fb;"><tr>
                    ...
                    // Build tbody rows safely using textContent
                    (order.items || []).forEach(item => {
                        ...
                        tdQty.textContent = (item.quantity || 0) + 'x';
                        const tdSubtotal = document.createElement('td');
                        tdSubtotal.className = 'text-end';
                        tdSubtotal.textContent = formatCurrency(item.subtotal || 0);
                    ...
                    safeSetHTML(leftBadge, qrisBadge);
                    ...
                    receiptIcon.className = 'bi bi-receipt me-1';
                    receiptBtn.appendChild(receiptIcon);
                    receiptBtn.appendChild(document.createTextNode('Lihat Struk'));
    ...
    if (isOrderPage) {
        ...
        function renderSearchResults(cat, filter = '') {
            ...
            results.forEach((res, i) => {
                ...
                // Build highlighted label safely without using innerHTML
                const parts = text.split(new RegExp(`(${q})`, 'gi')).filter(p => p !== '');
                parts.forEach(part => {
                    if (q && part.toLowerCase() === q.toLowerCase()) {
                        const mk = document.createElement('mark');
                        mk.style.backgroundColor = '#ffeb3b';
                        mk.style.fontWeight = 'bold';
                        mk.textContent = part;
                        item.appendChild(mk);
                    } else {
    ...
    if (isMyOrdersPage) {
        ...
        function displayMyOrders(ordersToShow = null) {
            ...
            finalOrders.forEach((order) => {
                ...
                const left = document.createElement('div');
                left.className = 'col-md-6';
                // Build left column safely
                const buyerP2 = document.createElement('p');
                buyerP2.className = 'mb-1';
                const buyerStrong2 = document.createElement('strong');
                buyerStrong2.textContent = '👤 Nama Pembeli:';
                buyerP2.appendChild(buyerStrong2);
                buyerP2.appendChild(document.createTextNode(' ' + (order.buyerName || '')));

                const tableP2 = document.createElement('p');
                tableP2.className = 'mb-1';
                const tableStrong2 = document.createElement('strong');
                tableStrong2.textContent = '🪑 Nomor Meja:';
                tableP2.appendChild(tableStrong2);
                tableP2.appendChild(document.createTextNode(' ' + (order.tableNumber || '')));

                const idP2 = document.createElement('p');
                idP2.className = 'mb-0';
                const idStrong2 = document.createElement('strong');
                idStrong2.textContent = '📅 ID Pesanan:';
                idP2.appendChild(idStrong2);
                const code2 = document.createElement('code');
                code2.textContent = order.id;
                idP2.appendChild(document.createTextNode(' '));
                idP2.appendChild(code2);

                left.appendChild(buyerP2);
                left.appendChild(tableP2);
                left.appendChild(idP2);

                const right = document.createElement('div');
                right.className = 'col-md-6 text-md-end';
                const statusWrapper2 = document.createElement('p');
                statusWrapper2.className = 'mb-3';
                statusWrapper2.innerHTML = statusBadge;
                const totalP2 = document.createElement('p');
                const totalStrong2 = document.createElement('strong');
                totalStrong2.textContent = '💰 Total:';
                const totalSpan2 = document.createElement('span');
                totalSpan2.style.color = '#17a2b8';
                totalSpan2.style.fontSize = '1.3rem';
                totalSpan2.style.fontWeight = 'bold';
                totalSpan2.textContent = formatCurrency(order.total);
                totalP2.appendChild(totalStrong2);
                totalP2.appendChild(document.createTextNode(' '));
                totalP2.appendChild(totalSpan2);
                right.appendChild(statusWrapper2);
                right.appendChild(totalP2);
                ...
                (order.items || []).forEach(item => {
                    const tr = document.createElement('tr');
                    const tdName = document.createElement('td'); tdName.textContent = item.itemName || '';
                    const tdCat = document.createElement('td'); tdCat.textContent = `(${item.category || ''})`;
                    const tdQty = document.createElement('td'); tdQty.className = 'text-end'; tdQty.textContent = (item.quantity || 0) + 'x';
                    const tdSubtotal = document.createElement('td'); tdSubtotal.className = 'text-end'; tdSubtotal.textContent = formatCurrency(item.subtotal || 0);
                    tr.appendChild(tdName); tr.appendChild(tdCat); tr.appendChild(tdQty); tr.appendChild(tdSubtotal);
